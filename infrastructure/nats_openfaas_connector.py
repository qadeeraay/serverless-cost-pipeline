#!/usr/bin/env python3
"""
🔗 NATS JetStream -> OpenFaaS Event Connector & Dispatcher

Subscribes to the S3-EVENTS JetStream stream, invokes the
image-processor-app function over HTTP for each event, ACKs on success,
and routes poison messages to the DLQ-POISON stream after 3 failed attempts.

Runs as a durable Kubernetes Deployment (infrastructure/nats-connector-deployment.yaml)
within the cluster to enforce at-least-once event delivery and asynchronous queue leveling.
